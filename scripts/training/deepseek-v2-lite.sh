...
export WEIGHTS_PATH=$WEIGHTS_DIR/deepseek-v2-lite/$JID/
mkdir -p $DATASET_PATH $WEIGHTS_PATH

export GCP_DATASET_PATH=$GCP_DATASET_DIR/$DATASET/tokenized/$TOKENIZER/
export GCP_WEIGHTS_PATH=$GCP_WEIGHTS_DIR/deepseek-v2-lite/$JID/

export MASTER_ADDR=$(hostname)
export MASTER_PORT=8000

srun -W 0 scripts/training/modules/remote_to_local.sh
srun -W 0 scripts/training/modules/deepseek-v2-lite_step1.sh
