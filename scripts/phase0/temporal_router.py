...
def compute_resident_mask(logits: torch.Tensor, k: int) -> torch.Tensor:
    ...
    Policy (use-then-swap, deployment-faithful):
        ...
        R_{t+1} = swap(R_t, logits[t]):             # the router nominates one expert to prefetch
            ...
            evicting the LRU resident (smallest last-refresh time).
    Refresh times: cold-fill experts rank by ascending logit (lowest = oldest); each nomination is
    the newest. All resident refresh times stay distinct, so eviction is deterministic.
    """
    S, B, E = logits.shape
    dev = logits.device
    ...
    refresh = torch.full((B, E), NEG, device=dev)           # last-refresh time per expert
    ...
    for t in range(S):
        ...
        do_swap = nom_val > worst_val                                   # R_t != global top-k
        evict_i = refresh.masked_fill(~resident, POS).argmin(dim=-1)    # LRU resident     [B]
        if do_swap.any():
            r = rows[do_swap]
            resident = resident.clone()
            resident[r, evict_i[do_swap]] = False
            resident[r, nom_i[do_swap]] = True
            refresh = refresh.clone()
            refresh[r, nom_i[do_swap]] = float(k + t)       # newest: strictly > any prior refresh
...
def install():
    ...
    print("[temporal] rolling-residency router installed (TopKRouter.forward patched)")
