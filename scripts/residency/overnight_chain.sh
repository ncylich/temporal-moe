...
#
#   ARMS=A,C scripts/residency/overnight_chain.sh     # subset
#   scripts/residency/overnight_chain.sh              # everything
set -uo pipefail
cd "$(dirname "$0")/../.."
...
_consolidate() {
  if ! "$PY" analysis/residency/consolidate.py; then
...
SEL=$("$PY" analysis/residency/pick_free_set.py "$DATA" "$BAR")
...
if _want E; then
  for spec in "$EXT:csurf_${EXT}_at200M.pt" "$WIN_TAG:csurf_${WIN_TAG}_at50M.pt"; do
    ...
    "$PY" analysis/residency/downstream.py --csurf "$ck" --free-set "$WIN_FS" --tag "$tg" \
          > "$LOGS/ds_${tg}.log" 2>&1
    rc=$?
    if [ $rc -ne 0 ]; then
