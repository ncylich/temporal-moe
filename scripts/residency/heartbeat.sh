...
echo "  GEMMA W=3 (05:31): R8 +3.6 = rebuild (+0.4 over it, z=0.4), 0 digit loops, best false-eq rate"
echo "  2.49%; R16 -0.4. Digit lever does nothing for gemma at any dose. SETTINGS: gemma W=1, qwen W=10."
echo "CORRECTED PATH (user, 08-28 06:00): (1) full surfaces for W=3 on BOTH models (queued); (2) NO"
echo "  replication seeds until the recipe is final; (3) prefer one recipe across models (W=3) over"
echo "  per-model tuning (W=10); (4) still short of the paper on qwen (GSM8K +5.4 vs +6.5, IFEval +0.2 vs"
echo "  +3.5, 4-cell +1.8 vs +2.7): next lever is self-distillation, run from the W=3 adapters."
echo "DISTILL ROUND 1 (09:38, KL-only from digit10): NO GAIN (R8 -0.6, free -1.1 z=-2.1). Cause: the"
echo "  samples were the PROSE lane only (selfgen_traj --n takes the pool in lane order; 0.7% digit"
echo "  chars) and KL-only dropped the free anchor. ROUND 2 (tmoe_{qwen,gemma}_onpol.sh): math/fewshot"
echo "  weighted samples from the W=3 student, CE+free anchor kept, --aux-traj on-policy KL added,"
echo "  from scratch at 3.4M. Launch AFTER the W=3 surfaces finish. Records *_ce_onpol3_n1319."
echo "W=3 SURFACES DONE (13:24): gemma 4-cell +1.6 = rebuild cell for cell (WB +0.053 best); qwen"
echo "  4-cell +1.7 (W=10 +1.8, rebuild -0.4). STANDING RECIPE: W=3 on both. Gap to paper: gemma +1.6"
echo "  vs +2.2 (HumanEval, GSM8K), qwen +1.7 vs +2.7 (IFEval, GSM8K). Round 2 onpol RUNNING."
echo "SERVING PATH REWRITTEN (08-28 18:20): fused Triton walker + CUDA graphs; R8 decode 432 ->"
echo "  4094 tok/s (9.5x), sampling 330 -> 4800 tok/s. Expect GSM8K n=1319 x3 arms in ~8 min, a full"
echo "  gemma surface in ~1h. TEMPORAL_EAGER=1 restores the old engine mode for paired checks."
echo "DISTILLATION REFRAMED (user, 08-28 19:15): samples from a DIFFERENT adapter are off-policy,"
echo "  and self-sampled trajectories call for REVERSE KL (sampled-token estimator), not forward."
echo "  mix (CE W=3 + anchor + fwd-KL on W=3-student samples, from scratch) finishes ~20:05 as an"
...
echo "  ORDER (user 08-28 21:45): after this run -> from-SCRATCH online reverse-KL (base under R8 as the"
echo "  initial student, 3.4M tokens, ~1h50m, record gemma4_ce_online_scratch_e16_n1319) -> gemma deadband"
echo "  surfaces (base rho=0.5, then W=3 rho=0.5). All armed as watchers. qwen only after a gemma win."
echo "CONTAINER OOM-KILLED 08-28 20:31 (limit 251 GB host RAM incl. /dev/shm): everything died, base"
echo "  model re-staged. RULE: one heavy process at a time, all under gpu_lease.sh (now waits for 140 GB"
echo "  RAM headroom). ONLINE SAMPLER: engine boot 210s, wake 2.5s, weight sync 0.6s; greedy parity vs"
echo "  merged ckpt was 2/8 in CUDA-graph mode -> eager parity + tensor-level diff queued (prio 0)."
echo "  e2e smoke held until parity is exact. Offline loop killed (samples kept)."
echo "GEMMA FIRST (user, 08-28 17:00): run every optimisation on gemma; replicate on qwen only if it"
echo "  works. qwen onpol dequeued (samples kept). AFTER gemma onpol: eviction deadband on gemma --"
echo "  base rho=0.5 full surface (tmoe_deadband_surface.sh gemma 0.5 /dev/shm/gemma4-26b-it gemma4_base),"
echo "  then the W=3 adapter + rho=0.5. If adapter+deadband is the best cell, iterate from there."
