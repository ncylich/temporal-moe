...
echo "ANALYTIC REVERSE-KL FROM SCRATCH (01:20): R8 82.5 (+3.7), free 86.9, R16 86.4 = sampled-token = W=3"
echo "  (all z<1). Three recipes stop at R8 82.3+/-0.3. SWEEP RUNNING (sweep_online.py, prio 4): baseline"
echo "  (analytic, anchor 0) -> lr1e-4 -> temp1.0 -> refresh8x128 -> budget6.8M; table results/ablations/"
echo "  online_sweep.md. Surface of the from-scratch adapter running (prio 3); qwen check queued; deadband after."
echo "STANDING FORMULATION (user 00:05): on-policy reverse-KL self-distillation replaces the W=3 CE recipe"
echo "  (model-agnostic, prompts only, no free-arm tax). Beat W=3 at R8 is the open target. qwen next,"
echo "  with no digit weight, after the analytic variant lands on gemma."
echo "FROM-SCRATCH ON-POLICY WORKS (23:50): reverse-KL only, no CE/data: R8 81.9 (+3.1; W=3 +3.6, z=-0.5),"
echo "  free 87.5 (-0.3), R16 87.0 (+0.4). KL trace flat 0.37 -> estimator suspected. RUNNING: analytic"
echo "  revkl_full from scratch (~01:25). USER 00:20: no two-stage variants (W=3->onpol cancelled); the"
echo "  from-scratch adapter gets the FULL SURFACE (queued, adapter-direct); setup work from scratch, one"
echo "  change each: anchor 0 first (CE is NOT applied in these runs; the 0.05 anchor was inherited),"
echo "  then lr, math-only prompts, rounds. qwen apply check + smoke queued; deadband after."
echo "RULE (user 23:40): a failed on-policy run gets a diagnosis and one-change fixes; stop only when the"
echo "  algorithm itself is the demonstrated limit. FROM-SCRATCH reverse-KL: KL flat 0.37 across the run,"
echo "  adapter norms ~= W=3 -> signal noisy, not weak. Fix ready: --aux-loss revkl_full (analytic reverse"
echo "  KL over the teacher top-50 + tail). Deadband watcher HELD until this line settles."
