#!/bin/bash
# FINAL protocol reruns: native reasoning path + budget overrides everywhere.
# Caps: thinking arms 4096 (IFEval 8192 -- essay task); non-thinking 2048.
set -euo pipefail
...
echo "### FR: qwen think-on $(date -u +%H:%M)"
...
$PY -u $G --model qwen35_instruct --path /dev/shm/qwen35-35b-a3b-instruct \
    ...
    --max-model-len 9728 --gpu-mem 0.94 --tasks "ifeval=200" \
    > /tmp/fr_qwen_on_ifeval.log 2>&1
echo "### FR: qwen think-on OK"
ci "qwen3.5 think-on: native reasoning path, budget-corrected"

echo "### FR: qwen think-off"
$PY -u $G --model qwen35_instruct --path /dev/shm/qwen35-35b-a3b-instruct \
