...
# ~4x longer so fewer prompts are covered). Sampler cap 4096 / context 6144. Evals with --think on at 16384-token caps:
...
MODEL=$1
export TMOE_PRIO=4 TMOE_KL_TEMP=2 TMOE_AUX_LOSS=revkl_full TMOE_ANCHOR_W=0 TMOE_BUDGET_ON=sampled TMOE_THINK=on TMOE_ONLINE_MML=6144
...
echo "### think-on $MODEL 1/2 train (full pool, 8.6M sampled tokens, thinking on, cap 4096) $(date -u +%H:%M)"
TMOE_NAME_SUFFIX=_think TMOE_MAXNEW=4096 bash $CH scratch 8600000 16 256 > /workspace/rerun-logs/${MODEL}_online_think.out 2>&1
