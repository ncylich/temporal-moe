...
# Then GSM8K n=1319 on free and R=k, adapter-direct, then the surface.
...
case $FAM in
  olmoe) B=/workspace/instruct-models/olmoe-0125-instruct; MODEL=olmoe_instruct; R=8; ARMS=free,R8; GEN=2048; MML=4096; OMML=2560; PFX=olmoe_ce_online ;;
  lfm)   B=/workspace/instruct-models/lfm25-8b-a1b;        MODEL=lfm25_instruct; R=4; ARMS=free,R4; GEN=4096; MML=8192; OMML=4096; PFX=lfm25_ce_online ;;
...
$L $PY -u analysis/residency/instruct_genbench_vllm.py --model $MODEL --path $B --adapter $A --arms $ARMS --record-as ${NAME}_full \
  --tasks "ifeval=0" --gen-cap 8192 --max-model-len 10240 --gpu-mem 0.90
$L $PY -u analysis/residency/mmlu_gptoss.py --model $MODEL --path $B --adapter $A --arms $ARMS --record-as ${NAME}_n_dual --gen-cap 4096 --max-model-len 6144 --gpu-mem 0.90
$L $PY -u analysis/residency/instruct_genbench_vllm.py --model $MODEL --path $B --adapter $A --arms $ARMS --record-as ${NAME}_code \
