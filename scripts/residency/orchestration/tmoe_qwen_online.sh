...
[ -f $A.done ] || { [ -n "$INIT" ] && cp $D/qwen_ce_${START}_adapter.pt $A
  $L $PY -u analysis/residency/train_gemma_ce.py $COMMON --out $A $INIT --accum 16 --lr ${TMOE_LR:-1e-4} --tokens $((SEEN+T)) \
    ...
    --online-every $EVERY --online-n $N --online-max-new ${TMOE_MAXNEW:-1024} --online-quota "${TMOE_QUOTA:-mathlane_v2=2341,d5_fewshot=1183,domain8k=1000}" --online-prompts ${TMOE_PROMPTS:-/workspace/olmoe-adapt/data/d7_prompts.jsonl} --online-temp ${TMOE_ONLINE_TEMP:-0.7} --budget-on ${TMOE_BUDGET_ON:-sampled} \
    --online-gpu-mem ${TMOE_ONLINE_MEM:-0.55} --online-offload ${TMOE_ONLINE_OFFLOAD:-20} --online-presence-penalty ${TMOE_ONLINE_PP:-1.5} --online-think ${TMOE_THINK:-off} --online-max-model-len ${TMOE_ONLINE_MML:-2560}
  touch $A.done; }
