#!/usr/bin/env bash
# After the gemma sweep runner finishes: (1) full surface (no WritingBench) on the gemma winner,
# (2-3) deadband surfaces (base rho=0.5, W=3 rho=0.5), (4) qwen on-policy from scratch at the sweep's best settings.
...
echo "### post-sweep 1/4 gemma full surface on the winner (no WB) $(date -u +%H:%M)"
...
echo "### post-sweep 2/4 deadband base rho=0.5 $(date -u +%H:%M)"
...
echo "### post-sweep 3/4 deadband W=3 rho=0.5 $(date -u +%H:%M)"
...
echo "### post-sweep 4/4 qwen on-policy from scratch $(date -u +%H:%M)"
