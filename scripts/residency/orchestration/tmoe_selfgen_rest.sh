#!/usr/bin/env bash
# Remaining selfgen surface (IFEval done at 16:25). Queued BEHIND the digit-weight tests.
set -euo pipefail
...
$L $PY -u analysis/residency/mmlu_gptoss.py --model gemma4_instruct --path $M --arms free,R8,R16 --record-as gemma4_ce_selfgen_full_dual --gpu-mem 0.90
echo "### selfgen HumanEval@8192 $(date -u +%H:%M)"
$L $PY -u analysis/residency/humaneval_gemma.py --path $M --arms free,R8,R16 --tag gemma4_ce_selfgen_he8192 --max-tokens 8192 --max-model-len 9216
echo "### selfgen MBPP@8192 $(date -u +%H:%M)"
$L $PY -u analysis/residency/mbpp_gemma.py --path $M --arms free,R8,R16 --tag gemma4_ce_selfgen_m8192 --max-tokens 8192 --max-model-len 9216 --gpu-mem 0.90
