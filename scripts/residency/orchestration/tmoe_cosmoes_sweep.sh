...
#   BlES sweep: COSMOES_LAMBDA in {0.1, 1, 10, 100} x GRAIN in {1, 3}, vanilla MoE
...
run_one () { # NAME GRAIN TEMPORAL COSMOES_LAMBDA
  ...
  GRAIN=$2 TEMPORAL=$3 TEMPORAL_EVICT=min_logit COSMOES_LAMBDA=$4 \
...
for G in 1 3; do
  ...
  for LAM in 0.1 1 10 100; do run_one g${G}_bles${LAM}_1e16 $G 0 $LAM; done
done
