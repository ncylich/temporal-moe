...
TMOE_NAME_SUFFIX=_fullpool bash /workspace/tmoe_gemma_online.sh scratch 4300000 16 256 > /workspace/rerun-logs/gemma_online_fullpool.out 2>&1
rc=$?; echo "### fullpool 1/4 done rc=$rc $(date -u +%H:%M)"; [ "$rc" = 0 ] || exit 1
echo "### fullpool 2/4 surface at 0.5x $(date -u +%H:%M)"
$S bash /workspace/tmoe_deadband_surface.sh gemma 0 adapter:$D/gemma_ce_online_scratch_e16_fullpool_adapter.pt gemma4_ce_online_fullpool_half > /workspace/rerun-logs/gemma_fullpool_half_surface.out 2>&1
echo "### fullpool 2/4 done rc=$? $(date -u +%H:%M)"
echo "### fullpool 3/4 stage 2: resume to 8.6M (1.0x coverage) $(date -u +%H:%M)"
TMOE_NAME_SUFFIX=_full bash /workspace/tmoe_gemma_online.sh online_scratch_e16_fullpool 4300000 16 256 > /workspace/rerun-logs/gemma_online_fullpool2.out 2>&1
rc=$?; echo "### fullpool 3/4 done rc=$rc $(date -u +%H:%M)"; [ "$rc" = 0 ] || exit 1
echo "### fullpool 4/4 surface at 1.0x $(date -u +%H:%M)"
$S bash /workspace/tmoe_deadband_surface.sh gemma 0 adapter:$D/gemma_ce_online_online_scratch_e16_fullpool_e16_full_adapter.pt gemma4_ce_online_fullpool_full > /workspace/rerun-logs/gemma_fullpool_full_surface.out 2>&1
