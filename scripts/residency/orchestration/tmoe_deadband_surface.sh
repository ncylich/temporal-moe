...
if [ "$MODEL" = gemma ]; then
  ARMS=${TMOE_ARMS:-R8,R16}; G="--model gemma4_instruct --path $M $ADAPTER --arms $ARMS"; ML=4096
  ...
  ARMS=${TMOE_ARMS:-R8,R32}; Q="--model qwen35_instruct --path $M $ADAPTER --arms $ARMS --think off --temperature 0.7 --top-p 0.8 --presence-penalty 1.5"
  echo "### $TAG GSM8K n=1319 $(date -u +%H:%M)"
