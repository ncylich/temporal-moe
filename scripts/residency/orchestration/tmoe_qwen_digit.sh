#!/usr/bin/env bash
# THE FIX AT THE SOURCE. Identical to the qwen rebuild (D7 pool, qwen's own trajectories,
...
[ -s "$A" ] || $L /workspace/venv_fla/bin/python -u analysis/residency/train_gemma_ce.py $COMMON \
  --accum 16 --lr 3e-5 --tokens 3400000 --kl-anchor $KL --kl-weight 0.1 --digit-weight $W
