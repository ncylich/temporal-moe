...
echo "### qwen smoke 1/2 reference dump (raw base + W=3 adapter via apply_adapter, same class as the sampler, eager) $(date -u +%H:%M)"
[ -s $S/ref_qwen_digit3_eager.json ] || $L $PY -u analysis/residency/parity_vllm.py --path /root/models/qwen35-35b-a3b --adapter /workspace/olmoe-adapt/data/qwen_ce_digit3_adapter.pt --R 8 --n 8 --max-new 256 --think off --out $S/ref_qwen_digit3_eager.json
...
$L $PY -u analysis/residency/train_gemma_ce.py --model /root/models/qwen35-35b-a3b --family qwen35 --no-unsloth --traj qwen35_d7_seq4096 --max-seq 4096 \
  ...
  --online-every 16 --online-n 32 --online-gpu-mem 0.55 --online-offload 20 --online-presence-penalty 1.5 --online-smoke $S/ref_qwen_digit3_eager.json
