...
dec() { echo "${1/p/.}"; }
case ${ARM%b} in                      # a trailing b names a same-seed replicate
  C0)    ENV="TEMPORAL_RESIDENCY_R=$E" ;;
  WK*C*) sw=${ARM%b}; sw=${sw#WK}; lam=$(dec ${sw#*C}); sw=${sw%%C*}; ENV="TEMPORAL_SWAPS=$sw TEMPORAL_COHERENCE_LAMBDA=$lam" ;;   # reuse policy + coherence loss on its own resident set
  ...
  WK*)   sw=${ARM%b}; ENV="TEMPORAL_SWAPS=${sw#WK}" ;;          # reuse-fraction arm: (k - swaps)/k of the previous token's experts kept
