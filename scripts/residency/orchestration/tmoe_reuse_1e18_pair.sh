#!/usr/bin/env bash
# After the running 1e18 arm (driver pid $1) finishes: run reuse 4/6 (WK2) at 1e18 too, take the lower
# final test CE of WK5 and WK2, and launch it at 1e19 if it is 0.010 below the recorded temporal
# triplet mean (3.9077). Log: curriculum_reuse_1e18b.out
set -uo pipefail; cd "$(dirname "$0")/../../.."
. scripts/env.sh
while kill -0 "$1" 2>/dev/null; do sleep 60; done
echo "### 1e18 WK5 finished $(date -u +%H:%M)"
ARM=WK2 GRAIN=1 bash scripts/residency/orchestration/tmoe_curriculum_1e18.sh
best=""; bestce=""
for a in WK5 WK2; do
