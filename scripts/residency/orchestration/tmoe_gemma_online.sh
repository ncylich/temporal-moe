...
[ -f $A.done ] || { [ -n "$INIT" ] && cp $D/gemma_ce_${START}_adapter.pt $A; rm -f $A.tmp
  $L $PY -u analysis/residency/train_gemma_ce.py $COMMON --out $A $INIT --accum 16 --lr 3e-5 --tokens $((SEEN+T)) \
    ${TMOE_CE:+--digit-weight ${TMOE_W:-3}} ${TMOE_CE:---kl-only} ${TMOE_ANCHOR_W:+$( [ "${TMOE_ANCHOR_W}" = 0 ] || echo "--kl-anchor $KL --kl-weight ${TMOE_ANCHOR_W}" )}${TMOE_ANCHOR_W:---kl-anchor $KL --kl-weight 0.05} --aux-loss ${TMOE_AUX_LOSS:-revkl} --aux-kl-weight ${TMOE_AUX_W:-1.0} \
