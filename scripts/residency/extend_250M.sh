...
for spec in "${CELLS[@]}"; do
  ...
  if [ -f "$DATA/ple_${tag}.json" ]; then
    ...
    "$PY" analysis/residency/consolidate.py > /dev/null || { echo "[FAIL] consolidate aborted" >&2; continue; }
  ...
  if [ ! -f "$DATA/$dck" ]; then
    ...
    "$PY" analysis/residency/downstream.py --csurf "$dck" --free-set "$fs" --tag "$tag" \
...
if [ -f "$DATA/csurf_ce_free_0_1_14_15_at50M.pt" ] && \
  ...
  "$PY" analysis/residency/downstream.py --csurf csurf_ce_free_0_1_14_15_at50M.pt \
