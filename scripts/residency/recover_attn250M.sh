...
while pgrep -f "analysis/residency/(train_ple|downstream)\.py" > /dev/null; do sleep 30; done
...
if [ -f "$DATA/ple_${TAG}.json" ]; then
  ...
  "$PY" analysis/residency/consolidate.py > /dev/null || { echo "[FAIL] consolidate" >&2; exit 1; }
