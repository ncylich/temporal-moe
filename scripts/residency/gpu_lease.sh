...
for i in $(seq 1 90); do
  ...
  # counted: anon + shmem (/dev/shm) + unevictable (pinned); page cache is reclaimable and excluded
  lim=$(cat /sys/fs/cgroup/memory.max 2>/dev/null)
  cur=$(awk '$1=="anon"||$1=="shmem"||$1=="unevictable"{s+=$2} END{print s+0}' /sys/fs/cgroup/memory.stat 2>/dev/null)
  if [ -n "$lim" ] && [ "$lim" != max ] && [ -n "$cur" ] && [ $(( (lim - cur) / 1073741824 )) -lt 140 ]; then
    [ $((i % 6)) -eq 1 ] && echo "[lease] waiting for host RAM: $(( (lim - cur) / 1073741824 )) GB non-reclaimable headroom of $((lim / 1073741824))"
    sleep 20; continue
  fi
