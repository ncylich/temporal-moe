...
"$PY" analysis/ple/consolidate.py > /dev/null 2>&1 && _gitsafe "record cells the queue completed before it was retired"
...
for spec in "${CELLS[@]}"; do
  ...
  if [ ! -f "$DATA/ple_${tag}.json" ]; then
    ...
    "$PY" analysis/ple/consolidate.py > /dev/null || { echo "[FAIL] consolidate" >&2; continue; }
