...
take the lazy-expert-load path and start in seconds.
    ...
    ./gate_ppl.py                # pread vs io_uring
    ./gate_ppl.py sqpoll         # pread vs io_uring+SQPOLL
"""
...
def arm(label, env):
    ...
    subprocess.run(["adb", "push", "/tmp/_gate_ppl.sh", f"{DEV}/_gate_ppl.sh"],
                   capture_output=True)
    out = sh(f"su -c 'sh {DEV}/_gate_ppl.sh'", timeout=3600)
...
if __name__ == "__main__":
    sq = "sqpoll" in sys.argv
    a = arm("A pread worker pool", "")
    b = arm("B io_uring" + (" SQPOLL" if sq else ""),
            "LLAMA_TEMPORAL_URING=1" + (" LLAMA_TEMPORAL_URING_SQPOLL=1" if sq else ""))
