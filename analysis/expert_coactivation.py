...
model_scale = "flame-moe-1.7b"
layer_nums = ["2", "6", "12", "18"]
steps = ["1100", "5500", "11029"]

K = 16
num_experts = 64
mount = "/tmp/slurm-31207"

# Worker to compute co-activation from a file
def worker(file: Path):
    probs = torch.load(file, map_location="cpu")
    _, expert_ids = probs.nonzero(as_tuple=True)
    data = expert_ids.view(probs.shape[0], 6)
...
# Compute co-activation matrix if not already done
def compute_and_save(step, layer_num):
    workspace = Path(mount, "actives", step, layer_num)
    output_file = Path(f"results/expert-coactivation/{model_scale}/{step}/{layer_num}.pkl")
    output_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    with ProcessPoolExecutor(max_workers=64) as executor:
        futures = [executor.submit(worker, file) for file in files]
        for future in tqdm(as_completed(futures), total=len(futures), desc=f"Processing {step}-{layer_num}", ncols=80):
            result = future.result()
            for i in range(num_experts):
                for j in range(num_experts):
                    record[i][j] += result[i][j]
...
for step in steps:
    for layer_num in layer_nums:
        compute_and_save(step, layer_num)
...
for step in steps:
    for layer_num in layer_nums:
        file = Path(f"results/expert-coactivation/{model_scale}/{step}/{layer_num}.pkl")
        with file.open('rb') as f:
            record = pickle.load(f)
        ...
        data_records.append((step, layer_num, filtered_record, top_expert_indices))
        global_min = min(global_min, filtered_record.min())
        global_max = max(global_max, filtered_record.max())

# Plotting (3 rows for steps, 4 columns for layers)
fig, axs = plt.subplots(3, 4, figsize=(24, 18))
axs = axs.flatten()

cmap = LinearSegmentedColormap.from_list("white_to_tartan", ["white", "#043673"])

for ax, (step, layer_num, filtered_record, top_expert_indices) in zip(axs, data_records):
    X, Y = np.meshgrid(np.arange(K + 1), np.arange(K + 1))
    ...
    ax.set_xticks(np.arange(K) + 0.5)
    ax.set_yticks(np.arange(K) + 0.5)
    ax.set_xticklabels(top_expert_indices, fontsize=8)
    ...
    ax.set_title(f"Step {step} - Layer {layer_num}", fontsize=10)

# Add a global colorbar
fig.colorbar(heatmap, ax=axs, shrink=0.6, pad=0.01, aspect=30, fraction=0.02)

# Save to PDF
out_file = Path(f"figures/expert-coactivation/{model_scale}/combined.pdf")
out_file.parent.mkdir(parents=True, exist_ok=True)
