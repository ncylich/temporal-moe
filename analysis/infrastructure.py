...
def plot(file: Path, mode: str, title: str, label: Optional[str] = None):
    ...
    sns.lineplot(data=df, x="x", y="y", label=label, linestyle="solid" if mode == "latency" else "dotted", marker="o")
    plt.title(title, fontsize=16)
    plt.gca().set_ylabel("Elapsed Time per Step (s)" if mode == "latency" else "Throughput (TFLOPS/s/GPU)", fontsize=16)
    plt.gca().set_xlabel("Training Step", fontsize=16)
