...
| # | status | headline |
|---|---|---|
| **N1** | **done, both directions** — *with a magnitude caveat* | The sham **reproduces** the endpoint shape in the impose direction (ends/interior **1.508×** sham vs **1.902×** real). Read as: the endpoint effect is largely positional, not lexical. **Magnitude caveat, now tested and resolved.** The random-R sham is 2.11× the real constraint's size (mean +0.596 vs +0.283), so comparing endpoint *ratios* between them assumed the profile scales linearly in perturbation magnitude. A second sham was built to test that: `TEMPORAL_SHAM=noise` adds Gaussian noise of tunable σ to the router logits and imposes no residency, calibrated to σ = 2.020 where the mean matches to **−0.2%** (+0.2824 vs +0.2828). Results in `sham_magnitude_matched.csv`:

| perturbation | mean | ends | interior | ends/interior |
|---|---|---|---|---|
| real constraint | +0.2828 | +0.4390 | +0.2308 | **1.902×** |
| sham, random-R | +0.5958 | +0.7974 | +0.5286 | 1.508× |
| sham, noise σ=1.93 | +0.2581 | +0.3460 | +0.2289 | 1.512× |
| sham, noise σ=2.020 | +0.2824 | +0.3804 | +0.2497 | **1.524×** |

**The ratio is magnitude-invariant**: three shams spanning a 2.3× range in mean cost give 1.508, 1.512 and 1.524. The linear-scaling assumption held, so the original comparison was sound.

**But the sham reproduces only 58% of the endpoint excess** — (1.524−1)/(1.902−1) — not all of it. And the residual is concentrated at the *last* layer. Real minus mean-matched sham, per layer: L2 **+0.022**, L3 −0.069, L4 −0.039, L5 −0.019, L6 +0.013, L7 −0.012, L8 +0.012, L9 **+0.095**. So the first MoE layer's excess is essentially fully positional (a lexicality-free perturbation reproduces it), while the last layer keeps ~0.095 nats — about 20% of its cost — that noise does not explain. Restated: **L2 is positional; L9 is mostly positional plus something specific to the constraint.** The unmask direction disagrees and should be discounted — there is no valid sham for "un-apply a perturbation the model never had", and its baseline is an off-distribution model at CE 4.895 vs native 3.909. |
| **N2** | **done, both directions**, now with `impose_all` | Endpoints add independently, interior does not: {2,9} ratio **1.02** imposing and **0.98** unmasking; {3–8} **0.81**. So single-layer costs predict the endpoint schedule and overstate an interior one by 23%. **With the full-imposition baseline** (`impose_all` on `flame38m_g1_moe`, CE 5.569861 vs native 3.918461, cost **+1.6514**): imposing on the interior {3–8} alone costs **+1.1240 = 68.1%** of full, and on the endpoints {2,9} alone **+0.8959 = 54.2%**. So **exempting the two endpoints buys back 31.9% of the constraint's cost for 25% of the layers** — better than proportional, but not dramatically. Note the two complementary sets sum to **1.22× the whole** (2.0198 vs 1.6514): constraining everywhere is *cheaper* than the sum of constraining the parts, so per-subset costs overstate a full schedule. |
