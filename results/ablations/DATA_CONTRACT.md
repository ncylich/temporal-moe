...
- gemma4 finals keep their recorded `mbpp_gemma` rows (same prompt and rule). The Qwen base
  ...
  from the stock lm_eval task (3-shot, primed fence, first block, 1536 budget). Every Qwen
  record with a stock MBPP row was re-measured under `mbpp_chat` (records `*_mbpp`,
  `tmoe_mbpp_qwen_all.sh`, adapters on the raw dir). Paired per item
  (`mbpp_stock_vs_unified.csv`) the unified protocol reads a median 1.4 points higher on
  non-thinking records and 6 points higher with thinking on (the primed fence suppressed
  thinking), so stock and unified MBPP rows are never mixed in one column: cite `*_mbpp`.
