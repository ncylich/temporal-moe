...
| `ple_results.csv` | PLE + per-layer-freeing program, ALL results in one tidy table (`group` column slices it: trained_cell / reference / parity / accounting / coverage / trainfree / calibration / cal_stack / layer_damage / free_set / locus / row_norms / heldout). Narrative: [ple_RESULTS.md](ple_RESULTS.md). | ple-adaptation, `analysis/ple/consolidate.py` |
| `ple_RESULTS.md` | PLE write-up: r=512 ties C@250M at a fifth of the tokens (5x token efficiency, 1 KB/token); freeing MoE layers 0/1/15 beats F' at +131% resident memory; mechanism claim REFUTED by the locus probe; calibrated init, LoRA stacking and sequential-vs-joint all null. | ple-adaptation closeout |
