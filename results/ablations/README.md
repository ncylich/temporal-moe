...
| `unmask_eval_1e19.csv` | Constraint swap at 1e19, eval-only: unmask temporal (+0.21/+0.20 BPB) and impose on moe_coarse (+0.43 BPB). **VERDICT 2026-08-03: all three cells re-derived and reproduce** — four of six values exact to four decimals, two within 3e-4. Superseded for new work by `cross_regime_eval.csv`, which records run name, checkpoint iteration and divisor per row. | h100@54c7d767, re-derived by `analysis/probes/cross_regime_eval.py` |
...
| `unmask_eval.csv` | Unmask / cross-regime eval — native vs cross-regime metric per scale & paradigm. **VERDICT 2026-08-03: partially unverifiable.** Five cells, not four: `flame38m` at 1e18 is a removal cell recorded in `val_CE`, which is why it is absent from the removal series usually quoted. It does not reproduce because the published figure is the validation split (+0.4853) and the re-derivation is the test split (+0.4954); in `test_BPB` it is +0.1663, which completes that series at four budgets (1e16 +0.0994, 1e17 +0.1242, 1e18 +0.1663, 1e19 +0.2006/+0.2065, monotone). `L0` and `minlogit` name runs this repository cannot identify — `L0` matches seventeen candidates — and `v16k`'s checkpoint is not on this machine, so those three can never be re-derived. Use `cross_regime_eval.csv` for new work. | a6000@d3fbc238, partially re-derived by `analysis/probes/cross_regime_eval.py` |
| `cross_regime_eval.csv` | Cross-regime evaluation cost, both directions, with provenance: run name, checkpoint iteration, both regime labels, CE, BPB and divisor per row; both arms scored on identical cached batches. Holds the four re-derivations above and four new g3-family cells that remove the confound in the imposition series — one family, two budgets, both directions: removal rises with budget (+0.1064 → +0.1850), imposition does not (+0.8884 → +0.8651). | `analysis/probes/cross_regime_eval.py` |
